...
export async function generateWorkflowFromSubmission(
  ...
  try {
    ...
    const existingGeneration = await prisma.aiWorkflowGeneration.findUnique({
      where: {
    ...
    const generationRecord = await prisma.aiWorkflowGeneration.create({
      data: {
        tenantId: ctx.tenantId,
        submissionId,
    ...
    try {
      ...
      await prisma.aiWorkflowGeneration.update({
