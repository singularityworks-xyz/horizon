...
import { validateTenantAccess, assertTenantAccess, validateUserHasTenantAccess, TenantAccess } from "./security/tenant"
...
export const rbac = {
  ...
  // NOTE: This trusts headers for convenience, but authorization decisions
  // must use validateTenantAccess() which re-validates from DB
  getTenantContext: async (): Promise<TenantContext | null> => {
    try {
      ...
      return { tenantId, userId, role } as TenantContext
    } catch (error) {
      return null
    }
  },
