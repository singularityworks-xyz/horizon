...
describe("validateAnswerAgainstConfig", () => {
  test("validates TEXT answers correctly", () => {
    ...
    expect(validateAnswerAgainstConfig({ type: "TEXT", value: "this is way too long" }, config)).toBe(false) // too long
  })
  ...
  test("validates MULTI_SELECT answers correctly", () => {
    ...
    expect(validateAnswerAgainstConfig({ type: "MULTI_SELECT", value: ["a", "b", "c"] }, config)).toBe(false) // too many
  })
  ...
  test("validates URL answers correctly", () => {
    ...
    expect(validateAnswerAgainstConfig({ type: "URL", value: "https://example.com" }, config)).toBe(true)
    expect(validateAnswerAgainstConfig({ type: "URL", value: "http://example.com" }, config)).toBe(false) // not https
...
describe("Zod Schemas", () => {
  ...
  test("SubmitAnswerSchema validates correctly", () => {
    const validAnswer = {
      questionId: "q1234567890123456789012345678901234567890", // cuid length
      value: {
        type: "TEXT",
        value: "John Doe",
      },
  ...
  test("CreateTemplateSchema validates correctly", () => {
    const validTemplate = {
      ...
      projectId: "p1234567890123456789012345678901234567890",
    }
  ...
  test("QuestionConditionSchema validates correctly", () => {
    const validCondition = {
      questionId: "q1234567890123456789012345678901234567890",
      operator: "equals",
      value: "yes",
    }
