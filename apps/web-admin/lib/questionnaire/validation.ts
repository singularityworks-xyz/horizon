...
export const QuestionConfigSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("TEXTAREA"), ...TextareaQuestionConfigSchema.shape }),
  z.object({ type: z.literal("SELECT"), ...SelectQuestionConfigSchema.shape }),
  z.object({ type: z.literal("MULTI_SELECT"), ...MultiSelectQuestionConfigSchema.shape }),
  z.object({ type: z.literal("FILE_UPLOAD"), ...FileUploadQuestionConfigSchema.shape }),
    ...
    case "URL": {
      ...
      try {
        ...
        if (urlConfig?.allowedDomains?.length) {
          return urlConfig.allowedDomains.some(domain => url.hostname === domain || url.hostname.endsWith(`.${domain}`))
        }
        return true
      } catch {
        return false
...
export function evaluateConditions(
  ...
  return conditions.every(condition => {
    ...
    switch (condition.operator) {
      case "equals":
        return answer.type === "SELECT" || answer.type === "TEXT" || answer.type === "TEXTAREA" || answer.type === "URL"
          ? answer.value === condition.value
          : answer.type === "NUMBER" ? answer.value === condition.value : false

      case "not_equals":
        return answer.type === "SELECT" || answer.type === "TEXT" || answer.type === "TEXTAREA" || answer.type === "URL"
          ? answer.value !== condition.value
          : answer.type === "NUMBER" ? answer.value !== condition.value : true
