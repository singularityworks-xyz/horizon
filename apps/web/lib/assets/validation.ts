...
export function validateFileType(mimeType: string): boolean {
  return ASSET_VALIDATION.ALL_ALLOWED_MIME_TYPES.includes(mimeType as any);
}
...
export function getFileCategory(mimeType: string): string {
  if (ASSET_VALIDATION.ALLOWED_MIME_TYPES.documents.includes(mimeType as any)) {
    return 'document';
  }
  if (ASSET_VALIDATION.ALLOWED_MIME_TYPES.images.includes(mimeType as any)) {
    return 'image';
  }
  if (ASSET_VALIDATION.ALLOWED_MIME_TYPES.archives.includes(mimeType as any)) {
    return 'archive';
  }
  return 'unknown';
}
