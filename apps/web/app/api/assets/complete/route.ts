...
export async function handleComplete(
  ...
  try {
    ...
    await prisma.asset_versions.create({
      data: {
...
async function validateLinkedEntities(data: AssetPresignData, tenantId: string): Promise<void> {
  const validations = [];
    ...
    validations.push(
      prisma.questions
        .findFirst({
          where: {
            id: data.questionId,
            questionnaire_templates: { tenantId },
          },
          select: { id: true },
        })
        .then((question) => {
